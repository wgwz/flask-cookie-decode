...
def test_decode(app):
    assert app.extensions['flask_decode'].decode(foo_cookie) == {'a': 'helloworld'}

def test_decode_with_timestamp(app):
    assert app.extensions['flask_decode'].decode(foo_cookie, return_timestamp=True) == ({'a': 'helloworld'}, datetime.datetime(2018, 12, 29, 21, 53, 52))

def test_compressed_decode(app):
    assert app.extensions['flask_decode'].decode(foo_compressed_cookie) == {
